	...
	public void createMatchObjects(ArrayList<String> values) {
		...
		for (int i = 0; i < totMatches; i++) {
			...
			if(!matchDuplicate(match)){
				matches.add(match);
				System.out.println("HEHEHEHEHHEHASJDOIAHSDOIAHSDOIHAOSIDHo");
...
//		System.out.println("Printing match.getHomeTeam(): " + match.getHomeTeam());
//		System.out.println("Printing match.getAwayTeam(): " + match.getAwayTeam());	
...
//				System.out.println("matchCheck: " + matchCheck);
	...
	public void assignProperties(Match match){
		System.out.println(match);
		match.assignProperties(match.getMinuteString(), match.getScore() , match.getMatchHandicap(), match.getTotalGoals(), match.getMatchAttach(), match.getMatchShoot());
		}
	
	public void setGraphData(Match match){
		
		System.out.println("Skriver ut all skit");
		System.out.println("minutes: " + match.getMinutes());
		System.out.println("home_score" + match.getHomeScore());
		System.out.println("home_attach" + match.getHomeAttach());
		System.out.println("home_shots" + match.getHomeShots());
		
		System.out.println("away_score" + match.getAwayScore());
		System.out.println("away_attach" + match.getAwayAttach());
		System.out.println("away_shots" + match.getAwayShots());
		match.setGraphData(match.getMinutes(), match.getHomeScore(), match.getAwayScore(), match.getHomeAttach(), match.getAwayAttach(), match.getHomeShots(), match.getAwayShots());
		}
	...
	public ArrayList<Match> liveMatch() {
		...
		for (Match m : matches) {
			...
			if (m.getMinutes() > -1) {
				...
				live_matches.add(m);
				// m.assignProperties();
				System.out.println("AISDJOIASJD" + m.getMinutes());
			}
		}
		print_live_matches();
		return live_matches;
...
//			System.out.println("away_shots" + m.getAwayShots());
	...
	public void calculateInconsistencies(Match match){
		System.out.println("testing calc.match: " + match);
		...
		System.out.println("testing graphData ended.");
